...
# #  Example: Parallel tempering for multimodal distributions vs. zeus
# 
...
# ## Now try it with zeus!

# In[21]:


import zeus 
print('zeus sampling (version: )', zeus.__version__)
...
# Run MCMC
p0 = 1 + np.random.rand(nwalkers, ndim)/10  # start near 1
start = p0
sampler.run_mcmc(start, nwarmup)
...
sampler = zeus.EnsembleSampler(nwalkers, ndim, log_posterior, moves=zeus.moves.GlobalMove())
# Run MCMC
...
# Plot the walker trajectories for the first parameter 
...
# Get the samples flattened
samples = sampler.get_chain(flat=True, discard=0, thin=1)
...
fig, axes = zeus.cornerplot(samples[::10], size=(8,8))


# In[27]:


# Try the zeus corner plot routine, using all
fig, axes = zeus.cornerplot(samples, size=(8,8))
