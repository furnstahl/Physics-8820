...
# 
# We used the affine-invariant sampler standalone `emcee` before. Here we'll compare to using another standalone sampler, `zeus`, and the probabilistic programming language `PyMC3`. 
...
starting_guesses = min_theta +                    (max_theta - min_theta) * np.random.rand(nwalkers,ndim)
